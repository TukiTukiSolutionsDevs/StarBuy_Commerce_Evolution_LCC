...
function NavItem({
  ...
  return (
    ...
    >
      ...
      {!collapsed && (
        <span className={`font-medium ${active ? 'text-[#d4a843]' : ''}`}>
          {label}
        </span>
      )}
      {active && !collapsed && (
    ...
    <div className="fixed inset-0 z-50 bg-[#0a0f1e] flex overflow-hidden">
      ...
      >
        ...
        <nav className={`flex-1 overflow-y-auto py-4 space-y-1 ${collapsed ? 'px-2' : 'px-3'}`}>
          {!collapsed && (
            <p className="text-[#374151] text-[10px] font-semibold uppercase tracking-widest mb-3 px-3">
              Navigation
            </p>
          )}
          {NAV_ITEMS.map((item) => (
        ...
        {/* Bottom actions */}
        <div className={`border-t border-[#1f2d4e] py-4 space-y-1 ${collapsed ? 'px-2' : 'px-3'}`}>
          {/* AI Chat toggle */}
          <button
